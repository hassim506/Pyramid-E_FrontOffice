...
export class SuperadminSondageComponent implements OnInit {
  ...
  goToPage(p: number): void { if (p < 1 || p > this.totalPages) return; this.currentPage = p; this.loadSondages(); }

  openCreate(): void { this.form = { type: 'satisfaction', declenchement: 'manuel', est_anonyme: false, est_obligatoire: false, est_actif: true, autorise_retour: true, afficher_progres: true, nombre_tentatives_max: 1, duree_estimee: 5 }; this.formError = ''; this.tab = 'creer'; }

  openEdit(s: Sondage): void { this.selectedSondage = s; this.form = { ...s }; this.formError = ''; this.tab = 'editer'; }
  ...
  saveSondage(): void {
    ...
    obs.subscribe({
      next: (res) => { this.saving = false; if (this.tab === 'creer' && res.sondage) { this.openQuestions(res.sondage); } else { this.tab = 'liste'; this.loadSondages(); } },
      error: (err) => { this.saving = false; this.formError = err?.error?.message ?? 'Erreur.'; },
    });
  }

  openQuestions(s: Sondage): void {
    this.selectedSondage = s; this.questions = [...(s.questions ?? [])]; this.editingQuestion = null; this.questionError = ''; this.tab = 'questions';
    this.sondageService.getSondage(s.id).subscribe({ next: (res) => { this.selectedSondage = res.sondage; this.questions = res.sondage.questions ?? []; } });
  }
  ...
  addOption(): void { if (!this.newOptionText.trim()) return; const opts = this.editingQuestion!.options as string[] ?? []; opts.push(this.newOptionText.trim()); this.editingQuestion!.options = opts; this.newOptionText = ''; }
  removeOption(i: number): void { (this.editingQuestion!.options as string[]).splice(i, 1); }

  saveQuestion(): void {
    if (!this.editingQuestion?.question || !this.editingQuestion?.type) { this.questionError = 'Champs obligatoires manquants.'; return; }
    ...
    obs.subscribe({ next: () => { this.editingQuestion = null; this.reloadQuestions(sid); }, error: (err) => { this.questionError = err?.error?.message ?? 'Erreur.'; } });
  }

  confirmDeleteQuestion(qId: number): void { this.confirmDeleteQId = qId; }
  deleteQuestion(): void {
    if (!this.confirmDeleteQId || !this.selectedSondage) return;
    this.sondageService.deleteQuestion(this.selectedSondage.id, this.confirmDeleteQId).subscribe({ next: () => { this.confirmDeleteQId = null; this.reloadQuestions(this.selectedSondage!.id); }, error: () => { this.confirmDeleteQId = null; } });
  }

  private reloadQuestions(sid: number): void { this.sondageService.getSondage(sid).subscribe({ next: (res) => { this.selectedSondage = res.sondage; this.questions = res.sondage.questions ?? []; } }); }

  openStats(s: Sondage): void { this.selectedSondage = s; this.statsLoading = true; this.stats = null; this.tab = 'stats'; this.sondageService.getStatistiques(s.id).subscribe({ next: (res) => { this.stats = res.statistiques; this.statsLoading = false; }, error: () => { this.statsLoading = false; } }); }

  toggleActif(s: Sondage): void { const obs = s.est_actif ? this.sondageService.archiveSondage(s.id) : this.sondageService.publishSondage(s.id); obs.subscribe({ next: () => this.loadSondages() }); }

  confirmDelete(id: number): void { this.confirmDeleteId = id; }
  deleteSondage(): void { if (!this.confirmDeleteId) return; this.sondageService.deleteSondage(this.confirmDeleteId).subscribe({ next: () => { this.confirmDeleteId = null; this.loadSondages(); }, error: () => { this.confirmDeleteId = null; } }); }
  ...
  getStatutClass(s: Sondage): string { if (!s.est_actif) return 'badge-inactif'; if (s.date_fin && new Date(s.date_fin) < new Date()) return 'badge-termine'; return 'badge-ouvert'; }
  getStatutLabel(s: Sondage): string { if (!s.est_actif) return 'Inactif'; if (s.date_fin && new Date(s.date_fin) < new Date()) return 'Terminé'; return 'Ouvert'; }
