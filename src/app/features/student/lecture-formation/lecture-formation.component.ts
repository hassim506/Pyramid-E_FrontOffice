...
export class LectureFormationComponent implements OnInit, OnChanges, OnDestroy {
  ...
  @Input()  embeddedCatalogueId:  number | null = null; // ✅ NOUVEAU — mode embarqué catalogue
  @Output() closePlayer = new EventEmitter<void>();
  get isEmbedded(): boolean { return this.embeddedFormationId !== null; }

  formationId!:  number;
  parcoursId:    number | null = null; // null = formation simple ou catalogue, X = dans parcours X
  catalogueId:   number | null = null; // ✅ NOUVEAU — null = formation simple ou parcours, X = dans catalogue X
  ...
  // ✅ Tous les getters passent parcoursId + catalogueId pour cibler le bon contexte
  //    formation simple   : parcoursId=null, catalogueId=null  → clé "42_null_null"
  //    dans parcours 5    : parcoursId=5,    catalogueId=null  → clé "42_5_null"
  //    dans catalogue 3   : parcoursId=null, catalogueId=3     → clé "42_null_3"
  ...
  ngOnInit(): void {
    if (this.embeddedFormationId) {
      ...
      if (idFromRoute) {
        ...
        // ✅ Récupérer parcoursId ET catalogueId depuis history.state
        //    → passés par parcours-assigne-detail ou catalogue-detail
        ...
        console.log(`🗝️  [LectureFormation] Clé ProgressionService sera: "${this.formationId}_${this.parcoursId ?? 'null'}_${this.catalogueId ?? 'null'}"`);
  ...
  loadStructure(): void {
    ...
    // ✅ Charge la progression dans le bon contexte (parcours, catalogue ou formation simple)
    ...
    this.formationsService.getFormationStructure(this.formationId).subscribe({
      next: (res: any) => {
        ...
        console.log(`📚 [LectureFormation] Modules (${this.modules.length}):`, this.modules.map(m => ({
          id: m.id, titre: m.titre, sections: m.sections?.length ?? 0
        })));
        ...
        console.log(`📋 [LectureFormation] Sections à plat (${this.flatSections.length}):`, this.flatSections.map(s => ({ id: s.id, titre: s.titre, type: s.type })));

        // ✅ Toujours passer parcoursId + catalogueId pour que le cache soit isolé
        this.progressionService.init(
          this.formationId,
          this.flatSections.length,
          [], // déjà chargées via loadFromApi
          this.parcoursId,
          this.catalogueId // ✅
        );

        console.log(`🔑 [LectureFormation] progressionService.init → formationId=${this.formationId} | totalSections=${this.flatSections.length} | parcoursId=${this.parcoursId} | catalogueId=${this.catalogueId}`);

        // Debug dump du cache après init
  ...
  selectSection(section: any): void {
    console.log(`📖 [LectureFormation] selectSection → id=${section.id} | titre="${section.titre}" | type=${section.type} | complétée=${this.isCompleted(section.id)}`);
