...
@Component({
  selector: 'app-adminrh-demande-session',
  standalone: true,
  imports: [RouterLink, FormsModule, CommonModule, MatSortModule, CustomPaginationComponent],
   templateUrl: './adminrh-demande-session.component.html',
  styleUrl: './adminrh-demande-session.component.scss'
})
export class AdminrhDemandeSessionComponent implements OnInit {
  routes = routes;
  ...
  validerDemande(demande: DemandeFormation, commentaire: string = ''): void {
    ...
    const confirmMessage = `Êtes-vous sûr de vouloir valider la demande de session "${demande.session_formation?.titre}" pour ${demande.employe?.name} ?`;
    ...
    if (confirm(confirmMessage)) {
      ...
      this.demandeFormationService.validerDemande(demande.id, data).subscribe({
        next: () => {
          this.successMessage = `Demande de session validée avec succès.`;
  ...
  refuserDemande(demande: DemandeFormation): void {
    ...
    this.demandeFormationService.refuserDemande(demande.id, { commentaire_rh: motif }).subscribe({
      next: () => {
        this.successMessage = `Demande de session refusée.`;
  ...
  annulerDemande(demande: DemandeFormation): void {
    const confirmMessage = `Êtes-vous sûr de vouloir annuler la demande de session "${demande.session_formation?.titre}" pour ${demande.employe?.name} ?`;
    ...
    if (confirm(confirmMessage)) {
      ...
      this.demandeFormationService.annulerDemande(demande.id).subscribe({
        next: () => {
          this.successMessage = `Demande de session annulée.`;
          this.refreshData();
        },
        error: (error) => {
