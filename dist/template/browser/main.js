...
var AuthInterceptor = (req, next) => {
  ...
  if (token && !req.url.includes("/login") && !req.url.includes("/register")) {
    console.log("\u{1F511} Adding token to request:", req.url);
    authReq = req.clone({
  ...
  return next(authReq).pipe(catchError((error) => {
    ...
    if (error.status === 401) {
      console.log("\u{1F6AB} Token expir\xE9 - Redirection en cours...");
      handleUnauthorized(roleRedirectService);
    }
    return throwError(() => error);
  }));
};
function handleUnauthorized(roleRedirectService) {
  const userData = localStorage.getItem("pyramide_user");
  if (userData) {
    try {
      const user = JSON.parse(userData);
      const roleId = user.role_id;
      console.log("\u{1F504} Nettoyage session et redirection pour role:", roleId);
      localStorage.removeItem("pyramide_token");
      localStorage.removeItem("pyramide_user");
      roleRedirectService.redirectByRole(roleId);
    } catch (error) {
      console.error("Erreur lors du parsing des donn\xE9es utilisateur:", error);
...
var AppComponent = class _AppComponent {
  ...
  constructor(router, inactivityService, authService) {
    ...
    this.router.events.subscribe((event) => {
      ...
      if (this.base === "lead dashboard" || this.base === "project dashboard" || this.base === "pipeline" || this.base === "payments" || this.base === "analytics" || this.base === "sources" || this.base === "lost reason" || this.base === "contact stage" || this.base === "industry" || this.base === "calls" || this.base === "manage users" || this.base === "roles permissions" || this.base === "permission" || this.base === "delete request" || this.base === "pages" || this.base === "roles permissions" || this.base === "testimonials" || this.base === "faq" || this.base === "contact messages" || this.base === "tickets" || this.base === "testimonials" || this.base === "faq" || this.base === "login" || this.base === "forgot password" || this.base === "email verification" || this.base === "two step verification" || this.base === "reset password" || this.base === "coming soon" || this.base === "under maintenance" || this.base === "layout fullwidth" || this.base === "layout hoverview" || this.base === "layout rtl" || this.base === "layout mini" || this.base === "layout hidden" || this.base === "layout dark" || this.base === "notifications") {
        this.page = this.base;
      }
    });
  }
  ...
  static \u0275cmp = /* @__PURE__ */ \u0275\u0275defineComponent({ type: _AppComponent, selectors: [["app-root"]], decls: 4, vars: 3, template: function AppComponent_Template(rf, ctx) {
    ...
    if (rf & 2) {
      \u0275\u0275advance(2);
      \u0275\u0275textInterpolate1("", \u0275\u0275pipeBind1(3, 1, ctx.page), " | CRMS - Advanced Bootstrap 5 Admin Template for Customer Management");
    }
