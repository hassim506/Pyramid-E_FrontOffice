...
var AdminrhSessionComponent = class _AdminrhSessionComponent {
  ...
  loadFormateurs() {
    this.userService.getFormateurs().subscribe({
      next: (res) => {
        this.formateurs = res.formateurs || res.data || (Array.isArray(res) ? res : []);
      },
      error: () => {
        this.userService.getMyUsers().subscribe({
          next: (res2) => {
            const all = res2.data || (Array.isArray(res2) ? res2 : []);
            this.formateurs = all.filter((u) => u.role === "formateur" || u.role_id === 2 || u.fonction?.toLowerCase().includes("formateur"));
          },
          error: () => {
          }
        });
...
//# sourceMappingURL=chunk-RFN2JZ64.js.map
