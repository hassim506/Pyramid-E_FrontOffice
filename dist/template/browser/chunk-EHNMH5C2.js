...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(StudentMesSondagesComponent, [{
    type: Component,
    args: [{ selector: "app-student-mes-sondages", standalone: true, imports: [CommonModule], template: `<div class="sms-wrap">

  <div class="sms-header">
    <h4 class="sms-title">Mes Sondages</h4>
    <p class="sms-sub">Participez aux sondages li\xE9s \xE0 vos formations</p>
  </div>
  ...
  <!-- Empty -->
  <div *ngIf="!loading && !error && sondages.length === 0" class="sms-empty">
    <i class="isax isax-note-21"></i>
    <p>Aucun sondage disponible pour le moment.</p>
    <span>Les sondages apparaissent apr\xE8s la fin d'une formation.</span>
  </div>

  <!-- Grid -->
  <div *ngIf="!loading && sondages.length > 0" class="sms-grid">
    <div *ngFor="let s of sondages" class="sms-card" (click)="openSondage(s)">

      <div class="sms-card__top">
        <span class="sms-badge" [class]="getDeclenchementClass(s.declenchement)">
          {{ getDeclenchementLabel(s.declenchement) }}
        </span>
        <span *ngIf="s.est_obligatoire" class="sms-badge sms-badge--required">Obligatoire</span>
      </div>

      <h6 class="sms-card__title">{{ s.titre }}</h6>
      <p class="sms-card__desc" *ngIf="s.description">{{ s.description }}</p>
      <p class="sms-card__desc sms-muted" *ngIf="!s.description">Aucune description</p>
      ...
      <button class="sms-btn">
        <i class="isax isax-play-circle me-2"></i> R\xE9pondre
      </button>
...
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && \u0275setClassDebugInfo(StudentMesSondagesComponent, { className: "StudentMesSondagesComponent", filePath: "src/app/features/student/student-mes-sondages/student-mes-sondages.component.ts", lineNumber: 14 });
})();
export {
  StudentMesSondagesComponent
};
//# sourceMappingURL=chunk-EHNMH5C2.js.map
